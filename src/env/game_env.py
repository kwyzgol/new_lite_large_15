...
class GameEnv:
    def __init__(
        ...
        self.player_distance: float = 0.0
    ...
    def reset(
        ...
        self.player_distance = self._calculate_player_distance()
    ...
    def step(self, action: Actions):
        ...
        for player in players_order:
            ...
            previous_distance = self.player_distance
            action = player.pending_action

            self._update_player_action(player)

            if (
                (is_move_action(action) or is_dash_action(action))
                and player.position != previous_position
            ):
                self._apply_distance_change_reward(player, previous_distance)
    ...
    def _calculate_player_distance(self) -> float:
        d_x = self.player1.col - self.player2.col
        d_y = self.player1.row - self.player2.row
        return (d_x ** 2 + d_y ** 2) ** 0.5

    def _apply_distance_change_reward(self, player: Player, previous_distance: float) -> None:
        current_distance = self._calculate_player_distance()

        if current_distance < previous_distance:
            player.add_reward(self.settings.reward_closer_distance)
        elif current_distance > previous_distance:
            player.add_reward(self.settings.penalty_greater_distance)

        self.player_distance = current_distance
