...
from src.agents.distributional_dqn_agent import DistributionalDqnAgent
...
def _save_model(agent: DistributionalDqnAgent, e_name: str, steps: int) -> str:
...
def _evaluate(
    ...
    env: GameEnv,
    agent: DistributionalDqnAgent,
    previous_agent: DistributionalDqnAgent | None,
...
def train_distributional_dqn(
    ...
    logger = TrainingLogger(
        ...
        settings=settings,
        algo_name=AlgoName.DISTRIBUTIONAL_DQN,
        algo_type=AlgoType.Q_VALUE,
    )

    previous_agent: DistributionalDqnAgent | None = None
